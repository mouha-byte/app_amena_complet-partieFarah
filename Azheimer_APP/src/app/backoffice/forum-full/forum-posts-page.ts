  ...
  template: `
    <div>
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px">
        <button type="button" (click)="back.emit()" style="color:#64748b;text-decoration:none;background:none;border:none"><i class="fa-solid fa-arrow-left"></i></button>
        <div>
      ...
      @if (loading) {
        ...
        @for (p of posts; track p.id) {
          <button type="button" (click)="openPost.emit(p.id)" style="text-decoration:none;color:inherit;display:block;margin-bottom:12px;background:none;border:none;padding:0;width:100%;text-align:left">
...
export class ForumPostsPage implements OnInit, OnChanges {
  ...
  constructor(private forumService: ForumIntegrationService, private authService: AuthService) {}
